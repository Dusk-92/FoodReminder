...
function ValidateChanges()
	buttonValider.MouseClick = function(sender, args)
			local tmpLine = tonumber(textBoxLines:GetText());
			if(tmpLine == nil)then
				tmpLine = tonumber(settings["nbrLine"]["nbr"]) or 1;
			end
			...
			local tmpSlots = tonumber(textBoxSlots:GetText());
			if(tmpSlots == nil)then
				tmpSlots = tonumber(settings["nbrSlots"]["nbr"]) or 7;
			end
